...
void toH(int no_of_disks, struct Stack*from, struct Stack *_using,struct Stack *to){
	...
	for (i = no_of_disks; i >= 1; i--) push(from, i);
	for (i = 1; i <= total_num_of_moves; i++){
		if (i % 3 == 1) moveDisksBetweenTwoRods(from, to, A, C);
		else if (i % 3 == 2) moveDisksBetweenTwoRods(from,_using, A, B);
		else if (i % 3 == 0) moveDisksBetweenTwoRods(_using, to, B, C);
	}
}
int main(){
